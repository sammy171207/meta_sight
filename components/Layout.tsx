...
const Layout: React.FC = () => {
  ...
  return (
    <div className="relative flex h-auto min-h-screen w-full flex-col group/design-root overflow-x-hidden font-display bg-background-light dark:bg-background-dark">
      <header className="sticky top-0 z-50 flex items-center justify-between whitespace-nowrap border-b border-solid border-b-[#e6ebf4] dark:border-b-gray-800 bg-white dark:bg-background-dark px-4 sm:px-10 lg:px-20 py-4 shadow-sm">
        <div className="flex items-center gap-4 text-[#0d121c] dark:text-white">
        ...
        <div className="hidden lg:flex flex-1 justify-end items-center gap-8">
          <nav className="flex items-center gap-9">
            <Link to="/" className={`text-sm leading-normal ${isActive("/")}`}>
              Home
            </Link>
            <Link to="/reports" className={`text-sm leading-normal ${isActive("/reports")}`}>
              Reports
            </Link>
            <Link to="/services" className={`text-sm leading-normal ${isActive("/services")}`}>
              Services
            </Link>
            <Link to="/about" className={`text-sm leading-normal ${isActive("/about")}`}>
              About Us
            </Link>
            <Link to="/blogs" className={`text-sm leading-normal ${isActive("/blogs")}`}>
              Blog
            </Link>
            <Link to="/press-releases" className={`text-sm leading-normal ${isActive("/press-releases")}`}>
              Press Releases
            </Link>
          </nav>
          <Link
      ...
      {/* Mobile Menu */}
      {isMenuOpen && (
        <div className="lg:hidden absolute top-16 left-0 w-full bg-white dark:bg-background-dark border-b border-gray-200 dark:border-gray-800 p-4 z-40 flex flex-col gap-4 shadow-lg">
          <Link to="/" className="text-sm font-medium" onClick={() => setIsMenuOpen(false)}>
            Home
          </Link>
          <Link to="/reports" className="text-sm font-medium" onClick={() => setIsMenuOpen(false)}>
            Reports
          </Link>
          <Link to="/services" className="text-sm font-medium" onClick={() => setIsMenuOpen(false)}>
            Services
          </Link>
          <Link to="/about" className="text-sm font-medium" onClick={() => setIsMenuOpen(false)}>
            About Us
          </Link>
          <Link to="/blogs" className="text-sm font-medium" onClick={() => setIsMenuOpen(false)}>
            Blog
          </Link>
          <Link to="/press-releases" className="text-sm font-medium" onClick={() => setIsMenuOpen(false)}>
            Press Releases
          </Link>
          <Link
            to="/contact"
